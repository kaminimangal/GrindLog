...
export default function History() {
  ...
  // Opens ConfirmModal — no delete yet
  ...
  async function confirmDelete() {
    ...
    setAllEntries(allEntries.filter(e => e.id !== deleteId))
    setDeleteId(null)
  }

  // Called by EditModal with { id, note, minutes, category_id }
  ...
  // ── Summary stats (from ALL loaded entries) ─────────────
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-3">
        ...
        <div className="flex items-center gap-4">
          <div className="relative">
            ...
            />
            <span
              className="material-symbols-outlined absolute left-2 top-1/2 -translate-y-1/2 text-text-muted"
              style={{ fontSize: '16px' }}
            >
              search
            </span>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className="border border-border rounded-lg px-4 py-3 bg-surface-low">
            ...
            <p className="text-text-primary text-xl font-semibold">
              {totalHours}<span className="text-text-muted text-xs font-normal ml-1">h</span>
            </p>
          </div>
          <div className="border border-border rounded-lg px-4 py-3 bg-surface-low">
            <p className="text-text-muted text-[10px] uppercase tracking-widest mb-1">Days Logged</p>
            <p className="text-text-primary text-xl font-semibold">
              {totalDays}<span className="text-text-muted text-xs font-normal ml-1">days</span>
            </p>
          </div>
          <div className="border border-border rounded-lg px-4 py-3 bg-surface-low">
          ...
          {hasMore && (
            <div className="flex justify-center pt-4">
              <button
                ...
                className="flex items-center gap-2 border border-border text-text-muted hover:text-text-primary hover:bg-surface-low px-6 py-2.5 rounded text-sm transition-all disabled:opacity-60"
              >
                {loadingMore ? (
                  <span
